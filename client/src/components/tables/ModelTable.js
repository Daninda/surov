...
class ModelTable extends Component {
	...
	render() {
		...
		return (
			<>
				<h1 className="table__title">Модели</h1>
				<Link className="add-button" to={"/create-model"}>
					Добавить
				</Link>
				<table className="table">
					...
					<tbody>
						{models.map((model) => {
							return (
								<tr key={model.model_id} className="table__body-row">
									<td>{model.model_id}</td>
									<td>{model.name}</td>
									<td>{model.color}</td>
									<td>{model.upholstery}</td>
									<td>{model.power}</td>
									<td>{model.doors}</td>
									...
									<td>
										<Link
											...
											to={`/update-model/${model.model_id}`}
										>
											<svg
									...
									<td>
										<Link
											className="remove-button"
											onClick={() => {
												DataService.deleteModel(model.model_id)
													.then(() => {
