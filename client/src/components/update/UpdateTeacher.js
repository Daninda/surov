...
class UpdateTeacher extends Component {
	...
	retrieve() {
		const teacher_id = +this.props.params.id;
		DataService.getTeacherRaw(teacher_id).then((res) => {
			this.setState(
				{
	...
	onClickSubmit() {
		...
		if (
			...
			DataService.updateTeacher(this.state.teacher_id, {
		...
		return (
			<>
				<table className="table teacher-table">
					<caption className="table__title">Обновление преподавателя</caption>
					<thead>
						<tr>
							<th width="20%">ФИО</th>
...
export default withRouter(withSnackbar(UpdateTeacher));
